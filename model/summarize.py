...
def get_parser():
    ...
    parser.add_argument("--beam_size", type=int, default=1, help="beam size") # parameter for beam search - epresents the number of paths or sequences to keep after each step of the model
    parser.add_argument("--length_penalty", type=float, default=1.0, help="") # control the length of the output sequences - 1.0 means no penalty
    parser.add_argument("--early_stopping", type=bool, default=False, help="") # stop the beam search when the best path is already sufficiently better than the current path
...
def main(params):
    ...
    reloaded = torch.load(params.model_path, map_location=torch.device('cpu')) # <= changed to cpu

    # print("Reloaded info: ", reloaded)
    ...
    encoder = TransformerEncoder(model_params, source_dico, with_output=False).eval()
    encoder.load_state_dict(reloaded['encoder'])
    decoder = TransformerDecoder(model_params, target_dico, with_output=True).eval()

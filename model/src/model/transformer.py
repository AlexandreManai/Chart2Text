...
def create_sinusoidal_embeddings(n_pos, dim, out):
    ...
    out_clone = out.clone()
    out_clone[:, 0::2] = torch.FloatTensor(np.sin(position_enc[:, 0::2]))
    out_clone[:, 1::2] = torch.FloatTensor(np.cos(position_enc[:, 1::2]))
    out_clone.detach_()
    out_clone.requires_grad = False
    return out_clone
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, params, dico, with_output):
        ...
        # embeddings
        # TODO remove the hardcode number --> ENCODING ERROR: 800 for pretrained model but only 1500 works for training the model
    ...
    def fwd(self, x1, x2, x3, x4, lengths):
        ...
        # print(positions.size(), slen, self.position_embeddings.weight.size(0))
